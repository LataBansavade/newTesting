...
// Update the route handler to use these functions
app.post("/match-drinks", upload.array("images", 10), async (req, res) => {
  try {
    const files = req.files || [];
    if (files.length === 0) return res.status(400).json({ error: "No images uploaded" });
...
`;
    ...
    // When constructing the final response, standardize preferences
    if (allDrinks.length > 0) {
      // Remove duplicates (same drink name)
      const uniqueDrinks = [];
      const drinkNames = new Set();
      ...
      // Construct the final response with standardized preferences
      const result = {
        status: "ok",
        preference: standardizePreferences(prefs),
        drinks: finalDrinks,
        sorted_by: "match_percentage (enforced)",
        notes: processingNotes.join(' | '),
        diagnostics: {
